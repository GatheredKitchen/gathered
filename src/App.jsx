        ...
        <div style={{minHeight:"100vh",background:B.black,display:"flex",flexDirection:"column",paddingTop:"env(safe-area-inset-top,20px)",paddingBottom:"env(safe-area-inset-bottom,20px)"}}>
          ...
          <div style={{flex:1,padding:"0 24px 32px"}}>
            <div style={{background:B.charcoal,border:`1px solid ${B.graphite}`,borderRadius:6,padding:"28px 22px"}}>
              ...
              {error && <div style={{background:"rgba(139,26,26,0.15)",border:"1px solid rgba(139,26,26,0.4)",borderRadius:3,padding:"10px 14px",fontSize:"0.8rem",color:"#E88080",marginBottom:16}}>{error}</div>}
              {success && <div style={{background:"rgba(107,124,92,0.15)",border:"1px solid rgba(107,124,92,0.4)",borderRadius:3,padding:"10px 14px",fontSize:"0.8rem",color:"#A8C49A",marginBottom:16}}>{success}</div>}
          ...
          <div style={{width:"100%"}}>
            ...
            <form onSubmit={handleSubmit}>
              {mode==="signup" && (
                <>
                  <label style={LS.lbl}>Your Name</label>
                  <input style={LS.inp} placeholder="Your name" value={name} onChange={e=>setName(e.target.value)} autoFocus/>
                </>
              )}
              <label style={LS.lbl}>Email Address</label>
...
function ShareModal({ recipe, user, allRecipes, onClose }) {
  ...
  const generateCardImage = async (downloadOnly=false) => {
    ...
    ctx.font = "300 24px sans-serif";
    const maxIng = 12;
    recipe.ingredients.slice(0, maxIng).forEach(ing => {
    ...
    const maxSteps = 8;
    recipe.instructions.slice(0, maxSteps).forEach((step, i) => {
      ctx.fillStyle = "#C9A84C";
      ctx.font = "300 28px serif";
      ...
      lines.forEach((l) => { ctx.fillText(l, 150, y); y += 32; });
      y += 8;
    });
        ...
        {mode === "single" && shareFormat === "card" && recipe && (
          <div style={{padding:"18px 24px"}}>
            ...
            <div style={{display:"flex", gap:10, marginTop:16}}>
              <button style={S.goldBtn} onClick={shareCardNative} disabled={generating}>{generating ? "Generating..." : "Share Card"}</button>
              <button style={S.outlineBtn} onClick={downloadCard} disabled={generating}>{generating ? "..." : "Download"}</button>
            </div>
          </div>
        )}
      ...
      <div style={S.recipeModal}>
        ...
        <div style={{background:B.graphite,padding:"24px 28px 20px",display:"flex",alignItems:"flex-start",gap:16,borderBottom:`1px solid ${B.smoke}`,flexWrap:"wrap"}}>
          ...
          <div style={{display:"flex",gap:6,flexShrink:0,flexWrap:"wrap"}}>
            ...
            <button onClick={()=>onDelete(recipe)} style={{...S.goldBtnSm, background:"none", color:"#C66", border:`1px solid #7A1515`}}>Delete</button>
            <button style={S.xBtn} onClick={onClose}>✕</button>
          </div>
        </div>
        {justScanned && (
          <div style={{padding:"14px 22px", background:"rgba(201,168,76,0.08)", borderBottom:`1px solid rgba(201,168,76,0.2)`, display:"flex", alignItems:"flex-start", gap:12}}>
            <div style={{flex:1, fontSize:"0.82rem", color:B.goldL, fontWeight:300, lineHeight:1.5}}>
              <strong style={{color:B.gold, fontWeight:600}}>Recipe scanned!</strong> Please review for accuracy. Tap <strong style={{color:B.gold}}>Edit</strong> to fix anything that looks off.
            </div>
      ...
      <div style={{...S.shareBox, maxWidth:600, maxHeight:"90vh", overflowY:"auto"}}>
        ...
        <div style={{padding:"24px 28px"}}>
          ...
          <div style={{marginBottom:14}}>
            ...
            <textarea style={{...inputStyle, minHeight:140, resize:"vertical"}} value={form.ingredients} onChange={e=>setForm({...form,ingredients:e.target.value})} />
          </div>
          <div style={{marginBottom:14}}>
            <label style={labelStyle}>Instructions (one step per line)</label>
            <textarea style={{...inputStyle, minHeight:140, resize:"vertical"}} value={form.instructions} onChange={e=>setForm({...form,instructions:e.target.value})} />
          </div>
          <div style={{marginBottom:18}}>
            <label style={labelStyle}>Notes</label>
            <textarea style={{...inputStyle, minHeight:70, resize:"vertical"}} value={form.notes} onChange={e=>setForm({...form,notes:e.target.value})} placeholder="Any special notes..." />
          </div>
          <div style={{display:"flex", gap:10}}>
            <button onClick={onClose} style={{flex:1, padding:"12px", background:"none", color:B.silver, border:`1px solid ${B.smoke}`, borderRadius:3, fontSize:"0.8rem", cursor:"pointer", fontFamily:"'Jost',sans-serif"}}>Cancel</button>
            <button onClick={handleSave} style={{flex:1, padding:"12px", background:B.gold, color:B.black, border:"none", borderRadius:3, fontSize:"0.8rem", fontWeight:600, cursor:"pointer", fontFamily:"'Jost',sans-serif"}}>Save Changes</button>
          </div>
        </div>
      </div>
    </div>
    ...
    <div style={S.overlay} onClick={e=>e.target===e.currentTarget&&onClose()}>
      <div style={{...S.shareBox, maxWidth:420}}>
        <div style={{padding:"32px 28px 20px", textAlign:"center"}}>
          <div style={{color:B.white, fontFamily:"'Cormorant Garamond',serif", fontSize:"1.6rem", fontWeight:300, marginBottom:10}}>Delete Recipe?</div>
          ...
          <div style={{color:B.mid, fontSize:"0.76rem", fontStyle:"italic"}}>This action cannot be undone.</div>
        </div>
        <div style={{padding:"0 28px 28px", display:"flex", gap:10}}>
          <button onClick={onClose} style={{flex:1, padding:"12px", background:"none", color:B.silver, border:`1px solid ${B.smoke}`, borderRadius:3, fontSize:"0.8rem", cursor:"pointer", fontFamily:"'Jost',sans-serif"}}>Cancel</button>
          <button onClick={()=>onConfirm(recipe)} style={{flex:1, padding:"12px", background:"#7A1515", color:B.white, border:"none", borderRadius:3, fontSize:"0.8rem", fontWeight:600, cursor:"pointer", fontFamily:"'Jost',sans-serif"}}>Delete</button>
        </div>
      </div>
    </div>
  );
    ...
    <div style={S.overlay} onClick={e=>e.target===e.currentTarget&&onClose()}>
      <div style={{...S.shareBox,maxWidth:440}}>
        ...
        <div style={{padding:"24px 32px"}}>
          ...
          <button onClick={onUpgrade} style={{width:"100%",padding:"14px",background:B.gold,color:B.black,border:"none",borderRadius:3,fontSize:"0.84rem",fontWeight:600,letterSpacing:"0.12em",textTransform:"uppercase",cursor:"pointer",marginBottom:10,fontFamily:"'Jost',sans-serif"}}>
            Upgrade to Pro
          </button>
          <button onClick={onClose} style={{width:"100%",padding:"10px",background:"none",color:B.mid,border:"none",fontSize:"0.76rem",cursor:"pointer",fontFamily:"'Jost',sans-serif"}}>
            Maybe later
          </button>
        </div>
      </div>
...
function MealPlannerTab({ recipes }) {
  ...
  const groceryPreview = {
    ...
    "Produce": ["4 carrots","4 potatoes","1 onion","1 lemon"],
    "Dairy & Refrigerated": ["3/4 cup heavy cream","6 tbsp butter"],
    "Pantry & Dry Goods": ["2 cups all-purpose flour","1/3 cup sugar","2 cups beef broth"],
    "Spices & Staples": ["Salt & pepper","1 tbsp baking powder","2 tbsp oil"],
  };
  return (
      ...
      <div style={{background:B.charcoal,border:`1px solid ${B.goldBd}`,borderRadius:6,marginBottom:44}}>
        <div style={{padding:"36px 44px"}}>
          ...
          <div style={{fontSize:"0.88rem",color:B.silver,fontWeight:300,lineHeight:1.8,marginBottom:30,maxWidth:640}}>
            Plan your week by dropping recipes onto any day. Gathered builds your grocery list automatically — ingredients combined, quantities scaled, grouped by aisle.
          </div>
          <div style={{display:"grid",gridTemplateColumns:"repeat(auto-fill,minmax(270px,1fr))",gap:10}}>
            {[
              ["Drag and drop recipes onto any day or week"],
              ["Export meal plan to Apple or Google Calendar"],
              ["AI grocery list — combined, scaled and aisle-sorted"],
      ...
      <div style={{display:"grid",gridTemplateColumns:"1fr 320px",gap:28,marginBottom:36}}>
        ...
        <div>
          ...
          <div style={{background:B.charcoal,border:`1px solid ${B.smoke}`,borderRadius:6,overflow:"hidden"}}>
            <div style={{padding:"14px 18px",borderBottom:`1px solid ${B.smoke}`}}>
              <div style={{fontFamily:"'Cormorant Garamond',serif",fontSize:"1.05rem",color:B.white}}>This Week's List</div>
              <div style={{fontSize:"0.68rem",color:B.mid,marginTop:2}}>Demo preview</div>
            </div>
...
export default function App() {
  ...
  const handleManageBilling = async () => {
    ...
    try {
      ...
      const data = await res.json();
      if (data.url) window.location.href = data.url;
    } catch { showToast("Couldn't open billing portal.", "err"); }
  };
  ...
  const handleUpload = async (e) => {
    ...
    try {
      ...
      if (savedRecipe) { setSelected(normalize(savedRecipe)); setJustScanned(true); }
  ...
  const handleDeleteRecipe = async (recipe) => {
    ...
    } else { showToast("Couldn't delete recipe. Try again.", "err"); }
  };

  const handleUpdateRecipe = async (recipeId, updates) => {
    const dbUpdates = {
      title: updates.title, category: updates.category, tags: updates.tags,
      servings: updates.servings, prep_time: updates.prepTime, cook_time: updates.cookTime,
      image: updates.image, ingredients: updates.ingredients, instructions: updates.instructions, notes: updates.notes,
    };
    ...
    if (!error && data) {
      ...
      if (selected && selected.id === recipeId) setSelected({ ...data[0], prepTime: data[0].prep_time, cookTime: data[0].cook_time });
      showToast("Recipe updated");
    } else { showToast("Couldn't update recipe. Try again.", "err"); }
  };
  ...
  const handlePhotoUpload = async (recipeId, file) => {
    ...
    try {
      ...
      if (selected && selected.id === recipeId) setSelected(prev => ({ ...prev, photo_url: photoUrl }));
  ...
  const scaleIngredient = (ingredient, multiplier) => {
    ...
    return ingredient.replace(/^(\d+\s+\d+\/\d+|\d+\/\d+|\d*\.\d+|\d+)/, (match) => {
      ...
      for (const [dec, frac] of Object.entries(fractions)) {
        if (Math.abs(decimal - parseFloat(dec)) < 0.05) return whole > 0 ? `${whole} ${frac}` : frac;
      }
  ...
  return (
    <>
      <style>{CSS}</style>
      <div style={{minHeight:"100dvh",display:"flex",flexDirection:"column",background:B.black}}>
        <header style={{minHeight:58,borderBottom:`1px solid ${B.graphite}`,display:"flex",alignItems:"center",padding:"12px 24px",flexShrink:0,gap:16}}>
        ...
        <div style={{flex:1,display:"flex",overflow:"hidden",position:"relative"}}>
          {tab==="Collection" ? (
            <>
              ...
              <aside style={{
                position:"fixed",top:"58px",left:0,bottom:0,width:196,
                ...
                <div style={{padding:"0 12px 12px"}}>
                  <div style={{fontSize:"0.56px",letterSpacing:"0.28em",textTransform:"uppercase",color:B.mid,marginBottom:8}}>Actions</div>
                  ...
                  <button onClick={()=>{ fileRef.current.click(); setSidebarOpen(false); }} disabled={uploading}
                    ...
                    {uploading ? "Scanning…" : "Add Recipe Photos"}
                  </button>
              ...
              <main style={{flex:1,overflowY:"auto",padding:"34px 38px"}}>
                ...
                {loadingRecipes ? (
                  ...
                  <div style={{textAlign:"center",padding:"100px 20px"}}>
                    <div style={{marginBottom:18,opacity:0.3,display:"flex",justifyContent:"center"}}><SprigMark size={40}/></div>
                    ...
                    <div style={{fontSize:"0.84rem",color:B.mid,fontWeight:300}}>Tap the menu then "Add Recipe Photos" to begin</div>
                  </div>
                ) : (
